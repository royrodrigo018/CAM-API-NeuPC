package com.dxc.imda.cam.neupc.mapper;
...
@Component
public class NeupcUserInfoMapper {
	
	private Logger logger = LoggerFactory.getLogger(this.getClass());
	
	private static final String PRIVILEGED_USER = "IMDA_SYS_ADMIN";
	...
	public UpdateInfo convertUpdateInfoToJSON(UserProfile userProfile) {
		...
		try {
			...
			updateInfo.setGroupId(userProfile.getUserRole().getRoleName());
	...
	private List<Group> getGroups(UserProfile userProfile){
		...
		group.setValue(userProfile.getUserRole().getRoleName());
		group.set$ref("");
		group.setDisplay(userProfile.getUserRole().getRoleDesc());
		groups.add(group);		
		return groups;
	}
	...
	private boolean isPrivileged(UserProfile userProfile) {
		...
		if (userProfile.getUserRole().getRoleName() != null && 
			userProfile.getUserRole().getRoleName().equalsIgnoreCase(PRIVILEGED_USER)) {
			isPrivileged = true;
		}
		return isPrivileged;		
	}
